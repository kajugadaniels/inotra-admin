...
const EventSubmissionScheduleVenue = ({ submission, isLoading, mapsApiKey }: Props) => {
    const latitude = toCoord(submission?.latitude);
    const longitude = toCoord(submission?.longitude);
                ...
                <div className="rounded-3xl border border-border/60 bg-background/60 p-6">
                    ...
                    <div className="mt-4 space-y-3 text-sm text-muted-foreground">
                        <div className="flex items-center gap-2">
                            ...
                            <span>Start: {formatDateTime(submission?.start_at)}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <CalendarRange className="h-4 w-4" />
                            <span>End: {formatDateTime(submission?.end_at)}</span>
                        </div>
                    </div>
                    {!submission && isLoading ? (
                        <p className="mt-4 text-xs text-muted-foreground">Loading schedule…</p>
                    ) : null}
                </div>
                    ...
                    <div className="mt-4 space-y-2 text-sm text-muted-foreground">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-semibold text-foreground">
                                {submission?.venue_name ?? "Venue TBA"}
                            </span>
                        </div>
                        {submission?.address ? <p>{submission.address}</p> : null}
                        <p>
...
export default EventSubmissionScheduleVenue;
