...
import { getEventSubmission } from "@/api/events";
import type { EventSubmissionDetail } from "@/api/types";
import { getApiBaseUrl } from "@/config/api";
import { Button } from "@/components/ui/button";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { EventDetailsSidebar } from "@/components/shared/events/details";
import {
    EventSubmissionOverview,
    EventSubmissionReviewDetails,
    EventSubmissionScheduleVenue,
    EventSubmissionTickets,
} from "@/components/shared/events/requests/details";
...
const EventSubmissionDetailsPage = () => {
    ...
    const [submission, setSubmission] = useState<EventSubmissionDetail | null>(null);
    ...
    useEffect(() => {
        ...
        if (!tokens?.access) {
            ...
            router.replace("/events/requests");
            return;
        }

        setIsLoading(true);
        getEventSubmission({ apiBaseUrl, accessToken: tokens.access, submissionId })
            .then((result) => {
    ...
    const tabs = [
        ...
        { key: "tickets", label: "Tickets", description: "Submitted ticket options" },
        ...
        <div className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">
                        {submission?.venue_name ?? "Venue"} · Starts {formatDate(submission?.start_at)}
                    </p>
                </div>
                <div className="flex items-center gap-2">
                    <Button asChild variant="outline" className="rounded-full h-11 text-xs">
                        <Link href="/events/requests">
                    ...
                    {submission?.approved_event_id ? (
                        <Button asChild variant="outline" className="rounded-full h-11 text-xs">
                            <Link href={`/events/${submission.approved_event_id}`}>
            ...
            <div className="rounded-3xl border border-border/60 bg-card/70 p-6 shadow-2xl shadow-black/5 backdrop-blur-xl">
                <Tabs defaultValue="overview" orientation="vertical" className="gap-6 lg:flex-row">
                    <EventDetailsSidebar tabs={tabs} title="Submission details" />
                    ...
                    <div className="space-y-6">
                        ...
                        <TabsContent value="overview">
                            <EventSubmissionOverview submission={submission} isLoading={isLoading} />
                        </TabsContent>
                        ...
                        <TabsContent value="tickets">
                            <EventSubmissionTickets submission={submission} isLoading={isLoading} />
                        </TabsContent>

                        <TabsContent value="review">
                            <EventSubmissionReviewDetails submission={submission} isLoading={isLoading} />
                        </TabsContent>
                    </div>
                </Tabs>
            </div>
...
export default EventSubmissionDetailsPage;
